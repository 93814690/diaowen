              ...
              <form action="${ctx }/sy/user/user-admin.action" method="post">
                <div style="padding-left: 30px;padding-top: 8px;padding-bottom: 8px;">
                  ...
                  <div style="padding: 5px;color: #666565;text-align: right;float: right;margin-right: 20px;">
                    <a href="${ctx }/sy/user/user-admin!input.action" class="user-plus active"><i class="fa fa-plus "
                                                                                                  aria-hidden="true"></i>&nbsp;添加账号</a>
                  </div>
                </div>
              </form>
            </div>
                      ...
                      <c:when test="${page.totalItems > 0}">
                        ...
                        <c:forEach items="${page.result }" var="en">
                          <tr>
                            ...
                            <td align="left"><fmt:formatDate value="${en.createTime }"
                            ...
                            <td align="center">
                              <a class="btn btn-default"
                                 href="${ctx }/sy/user/nosm/user-admin!input.action?id=${en.id}" title="编辑"
                                 data-toggle="tooltip" data-placement="top"><i class="fa fa-pencil-square-o"></i></a>
                              <a class="btn btn-default disUser_a"
                                 href="${ctx }/sy/user/nosm/user-admin!delete.action?id=${en.id}" title="禁用"
                                 data-toggle="tooltip" data-placement="top"><i class="fa fa-trash-o"
                  ...
                  <div style="padding-top: 15px;text-align: center;">
                    <div class="btn-group">
                      <c:if test="${page.pageNo > 1}">
                        <a href="${ctx }/sy/user/user-admin.action?page.pageNo=${page.pageNo-1}"
                      ...
                      <c:forEach begin="${page.startpage }" end="${page.endpage }" var="en">
                        <c:choose>
                          <c:when test="${page.pageNo eq en }"><a
                              href="${ctx }/sy/user/user-admin.action?page.pageNo=${en }" class="btn btn-default"
                              style="background: #D3DEED;">${en }</a></c:when>
                          <c:otherwise><a href="${ctx }/sy/user/user-admin.action?page.pageNo=${en}"
                                          class="btn btn-default">${en }</a></c:otherwise>
                        </c:choose>
                      </c:forEach>
                      <c:if test="${page.totalPage > (page.endpage)}">
                        ...
                        <a href="${ctx }/sy/user/user-admin.action?page.pageNo=${page.totalPage}"
                           class="btn btn-default">${page.totalPage }</a>
                      </c:if>
